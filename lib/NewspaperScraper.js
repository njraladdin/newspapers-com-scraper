...
class NewspaperScraper extends EventEmitter {
    ...
    // Rename search to retrieve
    async retrieve({ keyword, limit = null, dateRange = null, location = null }) {
        // Validate inputs
        if (!keyword || typeof keyword !== 'string') {
            throw new NewspaperScraperError('keyword is required and must be a string');
        }

        if (limit !== null && (!Number.isInteger(limit) || limit < 1)) {
            throw new NewspaperScraperError('limit must be null or a positive integer');
        }

        if (dateRange && (!Array.isArray(dateRange) || dateRange.length > 2)) {
            throw new NewspaperScraperError('dateRange must be an array of [year] or [startYear, endYear]');
        }

        // Calculate maxPages from limit
        const maxPages = limit ? Math.ceil(limit / this.config.resultsPerPage) : null;

        // Call the existing scraping method with the new parameters
        return this.scrapeNewspapers(keyword, maxPages, dateRange, location);
    }
}
